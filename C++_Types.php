...
if (isset($_POST['increase_progress'])) {
    ...
    if ($result) {
        header("Location: C++_Applications.php");
        exit();
    } else {
        // Handle the error
    ...
    <body>
        ...
        <?php
            ...
            if(mysqli_num_rows($selected) > 0){
                ...
                if ($progress < 30) {
                    echo '
                    <a href="#" class="active">Types of C++<br><span class="small">Current</span></a>
                    ';
                } else {
                    // Display the "Next" button
                    echo '<a href="C++_Types.php" class="active">Types of C++<br><span class="small">Completed</span></a>';
                }

                if ($progress < 40) {
                    echo '
                    <a href="#">Applications of C++</a>
                    ';
                } else {
                    // Display the "Next" button
        ...
        <div class="main">
        <h1 class="chead">C++</h1>
        <h2 class="subhead">Types of C++ :</h2>
        
        <p class="ccontent">C++ implementations are classified into four major categories, depending on the nature of the learning “signal” or “response” available to a learning system which are as follows:</p>
        ...
        <p class="ccontent">Supervised learning is the C++ task of learning a function that maps an input to an output based on example input-output pairs. The given data is labeled. Both classification and regression problems are supervised learning problems.</p>
        ...
        <p class="ccontent">A learner is not told what actions to take as in most forms of C++ but instead must discover which actions yield the most reward by trying them. For example — Consider teaching a dog a new trick: we cannot tell him what to do, what not to do, but we can reward/punish it if it does the right/wrong thing.</p>
        ...
        <p class="ccontent">Semi-supervised learning is an approach to C++ that combines small labeled data with a large amount of unlabeled data during training. Semi-supervised learning falls between unsupervised learning and supervised learning. </p>
        ...
        <p class="ccontent">C++ comes into the picture when problems cannot be solved using typical approaches.  C++ algorithms combined with new computing technologies promote scalability and improve efficiency.  Modern C++ models can be used to make predictions ranging from outbreaks of disease to the rise and fall of stocks.</p>
        ...
        <iframe class="cvideo" src="https://www.youtube.com/embed/olFxW7kdtP8"></iframe>
            ...
            if(mysqli_num_rows($selected) > 0){
                ...
                if ($progress < 30) {
                    ...
                    // Display the "Next" button
                    echo '<a href="C++_Applications.php"><button class="nbutton1" type="button">Next</button></a>';
                }
            }
            ?>
            </div>
